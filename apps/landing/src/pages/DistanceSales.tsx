    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="prose prose-lg text-gray-700 space-y-6">
          ...
          Ticari Unvanı: {brand}
          {address ? <><br />Adres: {address}</> : null}
          {phone ? <><br />Telefon: {phone}</> : null}
          {domain ? <><br />Web Sitesi: {domain}</> : null}</p>
          <p><strong>ALICI:</strong> Sipariş veren kişi.</p>

          <h2 className="text-xl font-bold text-gray-900">2. Sözleşmenin Konusu</h2>
          <p>İşbu sözleşme, ALICI'nın SATICI'ya ait {domain ? `${domain} ` : ''}web sitesi üzerinden elektronik ortamda sipariş verdiği ürünlerin satışı ve teslimatına ilişkin 6502 sayılı Tüketicinin Korunması Hakkında Kanun ve Mesafeli Sözleşmeler Yönetmeliği hükümleri gereğince tarafların hak ve yükümlülüklerini düzenler.</p>
