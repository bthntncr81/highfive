...
export async function expandBundles(
  ...
  for (const bundleReq of bundles || []) {
    ...
    // TEK satır: bundle wrapper. Sabit içerik + seçilen ek ürünler hepsi
    // modifier listesinde. Eski davranış (her alt ürün için 0₺ ayrı OrderItem)
    // iyzico online ödemeyi "basketItemPrice sıfırdan küçük veya eşit olamaz"
    // hatasıyla bloklarken POS/Kitchen ekranında aynı ürünler 2-3 kez tekrar
    // görünüyordu.
    const wrapperModifiers: string[] = [];
    for (const sel of selectedExtraLines) {
      wrapperModifiers.push(
        `${sel.slotLabel}: ${sel.menuItemName}${sel.extra > 0 ? ` (+${sel.extra}₺)` : ''}`,
      );
    }
