...
export default async function expenseCategoryRoutes(server: FastifyInstance) {
  ...
  server.get('/', { preHandler: verifyAuth }, async (request: FastifyRequest) => {
    ...
    const categories = await request.db.expenseCategory.findMany({
      where,
  ...
  server.put('/:id', { preHandler: verifyAdmin }, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const existing = await request.db.expenseCategory.findUnique({ where: { id } });
    ...
    const category = await request.db.expenseCategory.update({ where: { id }, data });
    return { category };
  });
  ...
  server.delete('/:id', { preHandler: verifyAdmin }, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const expenseCount = await request.db.expense.count({ where: { categoryId: id } });
    if (expenseCount > 0) {
      // Soft delete
      await request.db.expenseCategory.update({ where: { id }, data: { active: false } });
      return { ok: true, soft: true, message: 'Kategoriye bağlı gider var, pasif yapıldı' };
    }
    await request.db.expenseCategory.delete({ where: { id } });
    return { ok: true };
  });
}
